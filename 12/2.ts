...
lines.forEach((line, i) => {
  ...
  line.split("").forEach((char, j) => {
    currLineArr.push({
      ...
      value: char,
      visited: false,
      weight: char === "E" ? 0 : Number.MAX_SAFE_INTEGER
    })
...
//   if ((/* currSqr.value === "a" ||  */currSqr.value === "S") && col === startPos.column && smallestNumberOfSteps > currSteps) { smallestNumberOfSteps = currSteps; console.log("Found one of them!"); return; }
//   currSqr.visited = true;
//   let currSqrValue: number = currSqr.value === "E" ? "z".charCodeAt(0) : currSqr.value.charCodeAt(0);
...
//       const adjacentSqrValue: number = adjacentSqr.value === "S" ? "a".charCodeAt(0) : adjacentSqr.value.charCodeAt(0);
//       if (adjacentSqrValue + 1 !== currSqrValue && adjacentSqrValue !== currSqrValue) return;

//       traceRecursive(adjacentSqrPos.line, adjacentSqrPos.column, currSteps + 1);
//     });
//   currSqr.visited = false;
// }

// traceRecursive(finalPos.line, finalPos.column, -2, 0);


console.log("I DID IT MANUALLY BASED ON PT 1");